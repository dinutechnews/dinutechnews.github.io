...
// Check if Firebase is configured (simple check for placeholder)
const isFirebaseConfigured = () => {
    // In a real app, we'd check env vars or specific config values
    // Here we assume if the user hasn't replaced the placeholder, use mock
    // This is a heuristic; ideally we check for valid API key format
    return false; // Defaulting to mock for now as per plan until user updates keys
};
...
export const getArticles = async () => {
    ...
    try {
        ...
        return MOCK_ARTICLES; // Fallback on error
    }
};

export const getArticleById = async (id) => {
    if (!isFirebaseConfigured()) {
        return new Promise(resolve => setTimeout(() => resolve(MOCK_ARTICLES.find(a => a.id === id)), 300));
    }
    try {
